...
public class SlideshowGenerator {
    ...
    static public void main(final String[] args) throws Exception {
        ...
        final List<FileRecord> records = new LinkedList<>(file2FileRecords.values().stream()
            .filter(fr -> isParentUri(srcUri, uncheck(() -> new URI(fr.path()))))
...
//            .build();
        ...
        for(final var group: grouped) {
            ...
            if(group.size() < 2) {
                copyForSlideshow(group.get(0), srcUri, dstDir);
                continue;
            }
            ...
            final FileRecord[] toCopy = selectImagesFromGroup(sorted, 0.99);
            if(toCopy != null && toCopy.length > 0) {
                Arrays.stream(toCopy)
                    .forEach(fr -> uncheck(() -> copyForSlideshow(fr, srcUri, dstDir)));
            }
        }
    }

    private static double[][] correlationMatric(final List<FileRecord> src) {
        try(final Closer closer = new Closer();) {
            final CvMat[] mats = src.stream()
                .map(fr -> closer.add(uncheck(() -> loadImageWithCorrectOrientation(fr, vfs))))
                .toArray(CvMat[]::new);
            ...
            for(int i = 0; i < src.size(); i++) {
                for(int j = i + 1; j < src.size(); j++) {
                    ...
                    if(m1 == null || m2 == null)
                        ...
                        ret[i][j] = ImageUtils.correlationCoef(m1, m2);
                }
            }
            // fill in the rest
            for(int i = 0; i < src.size(); i++)
    ...
    private static FileRecord[] selectImagesFromGroup(final List<FileRecord> selectFrom, final double thresh) {
        final double[][] corrMat = correlationMatric(selectFrom);
...
//            return null;
        ...
        final FileRecord dng = toChooseFrom.stream()
            .filter(fr -> fr.path().toLowerCase().endsWith(".dng"))
            .findFirst()
            .orElse(null);
        if(dng != null)
            return dng;
        ...
        final FileRecord cr2 = toChooseFrom.stream()
            .filter(fr -> fr.path().toLowerCase().endsWith(".cr2"))
            .findFirst()
            .orElse(null);

        if(cr2 != null) {
            // see if there's a jpg of the same name.
            final int extIndex = cr2.path().toLowerCase().lastIndexOf(".cr2");
            final String fnameToLookForLc = cr2.path().toLowerCase().substring(0, extIndex) + ".jpg";
            final FileRecord cr2Jpg = toChooseFrom.stream()
                .filter(fr -> fr.path().toLowerCase().equals(fnameToLookForLc))
                .findFirst()
                .orElse(null);
    ...
    private static void copyForSlideshow(final FileRecord toCopy, final URI srcDir, final File destDirX)
        ...
        try(CvMat mat = loadImageWithCorrectOrientation(toCopy, vfs);) {
            if(mat == null) {
                System.err.println("Failed to read image at " + toCopy.path());
                return;
            }
            if(toCopy.mime().contains("image/jpeg") && mat.rows() <= 1080 && mat.cols() <= 1920) {
                ...
                System.out.println("Copying " + toCopy.path() + " to " + dest);
                ...
                try(CvMat resized = ImageUtils.resizeIfTooLarge(mat, 1080, 1920);) {
                    System.out.println("Copying resized image from " + toCopy.path() + " to " + destFile);
                    destFile.getParentFile().mkdirs();
